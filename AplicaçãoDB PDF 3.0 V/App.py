...
def exportar_para_pdf(treeview, nome):
    # Gerar nome do arquivo com base no parâmetro passado
    data_atual = datetime.datetime.now().strftime("%d-%m-%Y %Hh%Mm")
    arquivo_pdf = f"{nome}_{data_atual}.pdf"

    # Criar o documento PDF
    pdf = SimpleDocTemplate(arquivo_pdf, pagesize=letter)
    elements = []

    # Definir o cabeçalho da tabela com base nas colunas da treeview
    headers = [treeview.heading(col)["text"] for col in treeview["columns"]]

    # Extrair os dados da treeview
    data = [headers]  # Adiciona o cabeçalho como a primeira linha

    # Iterar pelas linhas da treeview
    ...
    style = TableStyle([
        ...
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),  # Alinhar texto no centro
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),  # Fonte do cabeçalho em negrito
        ('BOTTOMPADDING', (0, 0), (-1, 0), 12),  # Espaçamento no cabeçalho
        ('BACKGROUND', (0, 1), (-1, -1), colors.beige),  # Cor de fundo das células
        ('GRID', (0, 0), (-1, -1), 1, colors.black),  # Adicionar bordas
    ])

    table.setStyle(style)

    # Adicionar a tabela aos elementos do PDF
    elements.append(table)

    # Gerar o PDF
    pdf.build(elements)
    messagebox.showinfo('PDF', f"Arquivo gerado: {arquivo_pdf}")
...
class Application:
    def __init__(self, master=None):
        ...
        self.title_label = Label(self.central_frame, text="Menu Principal", font=("Helvetica", 18, "bold"),
                                 bg='#ecf0f1', fg='#2c3e50')
    ...
    #    self.frame_usuarios.configure(bg='#34495e')
    #    self.frame_clientes.configure(bg='#34495e')
    #    self.frame_cidades.configure(bg='#34495e')

        back_button = Button(frame, text="Voltar para Menu Principal",font=("Helvetica", 18, "bold"),
                                 bg='#ecf0f1', fg='#2c3e50', command=self.create_main_menu)
        back_button.pack(pady=10)

    def add_pdf_button(self, frame, treeview, nome):
        pdf_button = Button(frame, text="Exportar para PDF", font=("Helvetica", 18, "bold"),
                            bg='#ecf0f1', fg='#2c3e50', command=lambda: exportar_para_pdf(treeview,nome))
    ...
    def add_usuarios_content(self):
        ...
        self.add_pdf_button(self.frame_usuarios, self.tree_usuarios,"usuarios")
    ...
    def add_cidades_content(self):
        ...
        self.add_pdf_button(self.frame_cidades, self.tree_cidades, "Cidades")
    ...
    def add_clientes_content(self):
        ...
        self.add_pdf_button(self.frame_clientes, self.treeview, "Clientes")
    ...
    def inserirUsuario(self):
        ...
        messagebox.showinfo('Inserir',self.lblmsg["text"])
    ...
    def alterarUsuario(self):
        ...
        messagebox.showinfo('Alterar',self.lblmsg["text"])
        self.limparCamposUsuario()
        self.refresh_usuarios_treeview()
    ...
    def excluirUsuario(self):
        ...
        messagebox.showinfo('Deletar',self.lblmsg["text"])
        self.limparCamposUsuario()
        self.refresh_usuarios_treeview()
    ...
    def buscarUsuario(self):
        ...
        messagebox.showinfo('Buscar',self.lblmsg["text"])
        if user.idusuario:
    ...
    def inserirCidade(self):
        ...
        messagebox.showinfo('Inserir',self.lblmsgCidade["text"])
    ...
    def alterarCidade(self):
        ...
        messagebox.showinfo('Alterar',self.lblmsgCidade["text"])
        self.limparCamposCidade()
        self.refresh_cidades_treeview()

    def excluirCidade(self):
        cidade = Cidades(idcidade=self.txtidcidade.get(), nome=self.txtnomecidade.get())
        self.lblmsgCidade["text"] = cidade.deleteCidade()
        messagebox.showinfo('Excluir',self.lblmsgCidade["text"])
        self.limparCamposCidade()
    ...
    def buscarCidade(self):
        ...
        messagebox.showinfo('Buscar',self.lblmsgCidade["text"])
        if cidade.idcidade:
    ...
    def inserirCliente(self):
        cliente = Clientes(nome=self.txtnomecliente.get(), telefone=self.txttelefonecliente.get(), email=self.txtemailcliente.get(),  cidades= self.cidade_combobox.get())
        self.lblmsgCliente["text"] = cliente.insertCliente()
        messagebox.showinfo('Inserir',self.lblmsgCliente["text"])
        self.limparCamposCliente()
        self.refresh_clientes_treeview()
    def alterarCliente(self):
        cliente = Clientes(idcliente=self.txtidcliente.get(), nome=self.txtnomecliente.get(), telefone=self.txttelefonecliente.get(), email=self.txtemailcliente.get(), cidades= self.cidade_combobox.get())
        self.lblmsgCliente["text"] = cliente.updateCliente()
        messagebox.showinfo('Alterar',self.lblmsgCliente["text"])
        self.limparCamposCliente()
        self.refresh_clientes_treeview()
    def excluirCliente(self):
        cliente = Clientes(idcliente=self.txtidcliente.get())
        self.lblmsgCliente["text"] = cliente.deleteCliente()
        messagebox.showinfo('Excluir',self.lblmsgCliente["text"])
        Message
        self.limparCamposCliente()
        self.refresh_clientes_treeview()
    def buscarCliente(self):
        ...
        messagebox.showinfo('Buscar',self.lblmsgCliente["text"])
