...
class Clientes:
    def __init__(self, idcliente=0, nome="", telefone="", email="", endereco="", cidades=""):
    ...
    def insertCliente(self):
        ...
        try:
            ...
            c.execute("""
            INSERT INTO clientes (nome, telefone, email, endereco, cidades)
            VALUES (?, ?, ?, ?, ?)
            """, (self.nome, self.telefone, self.email, self.endereco, self.cidades))
    ...
    def updateCliente(self):
        ...
        try:
            ...
            c.execute("""
            UPDATE clientes
            SET nome = ?, telefone = ?, email = ?, endereco = ?, cidades = ?
            WHERE idcliente = ?
            """, (self.nome, self.telefone, self.email, self.endereco, self.cidades, self.idcliente))
            banco.conexao.commit()
            c.close()
